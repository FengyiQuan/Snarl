...
class MyApplication(arcade.Window):
    ...
    def on_draw(self):
        ...
        if self.player_sprite.alive:
            ...
            arcade.draw_rectangle_filled(x - math.ceil((24 - (self.health / 4.16)) / 2), y - 16,
                                         math.ceil(self.health / 4.16), 4, (0, 255, 0))

        # Death screen
        if not self.player_sprite.alive:
            arcade.draw_text("You died!", self.player_sprite.center_x - 125, self.player_sprite.center_y, (200, 20, 20),
                             48)
            arcade.draw_text("Press R To Restart", self.player_sprite.center_x - 125, self.player_sprite.center_y - 125,
                             (200, 20, 20), 24)

        # Instructions
        if not self.game_started:
            arcade.draw_texture_rectangle(self.player_sprite.center_x, self.player_sprite.center_y, 150, 200,
                                          self.controls)
    ...
    def update(self, delta_time):
        ...
        if changed:
            arcade.set_viewport(self.view_left, SCROLL_WIDTH + self.view_left, self.view_bottom,
                                SCROLL_HEIGHT + self.view_bottom)
